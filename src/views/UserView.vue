...
watch(
  ...
  async (newId) => {
    ...
    console.log(uid);
    const cid = parseInt(
      Object.keys(contents.value).find((x) => {
    ...
    if (
      ...
      await sendRequest<GetUserPageResult>(search).then((data) => {
...
<template>
  <main class="h-full flex flex-col">
        ...
        <div class="grow flex flex-col">
          <div class="flex gap-2">
            <div class="text-3xl font-bold">
              {{ users[userId]?.username || "Unknown" }}
            </div>
            <div class="text-l" v-show="users[userId]?.super === 1">⭐️</div>
            <div>(uid: {{ users[userId]?.id }})</div>
          </div>
          <div>
            Join Date:
            {{ new Date(users[userId]?.createDate).toLocaleDateString() }}
          </div>
          <div></div>
          <span>
            [<router-link :to="`/edit-page/${contentId}`"
              >Edit Userpage</router-link
            >]
          </span>
        </div>
      </div>
    ...
    <div v-else>Couldn't find user or page</div>
  </main>
</template>
